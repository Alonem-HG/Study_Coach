...
// ====== TTS helpers (persisten entre renders) ======
...
function pickVoice(lang) {
    ...
    return (
        VOICE_CACHE.find((v) => v.lang?.toLowerCase().startsWith(lower)) ||
        VOICE_CACHE.find((v) =>
            lower.startsWith("es") ? /spanish/i.test(v.name || "") : /english/i.test(v.name || "")
        ) ||
        VOICE_CACHE[0]
    );
}
...
export default function StudyCoach() {
    ...
    // Utils TTS (versión robusta para Android WebView)
    const stopSpeak = () => {
        try {
            window.speechSynthesis.cancel();
        } catch {}
    ...
    const speak = async (text, opts = {}) => {
        ...
        try {
            ...
            // Cancelar cualquier lectura previa y “despertar” el motor
            synth.cancel();
            synth.resume();

            const utter = new SpeechSynthesisUtterance(text);
            // Si parece inglés puro, usa en-US; por defecto es-ES
            const guessEn = /[a-z]/i.test(text) && !/[áéíóúñü]/i.test(text);
            utter.lang = opts.lang || (guessEn ? "en-US" : "es-ES");
            utter.rate = opts.rate || 1;

            const v = pickVoice(utter.lang);
            if (v) utter.voice = v;

            // Pequeño delay ayuda en algunos Android
            setTimeout(() => {
                try {
                    synth.resume();
                    synth.speak(utter);
    ...
    return ( <
...

        {
            tab === "practica" && current && ( <
                ...
                /label> {
                    repeatLeft > 0 && ( <
                        span className = "repeat-left" > Pendiente: {
                            repeatLeft
                        } < /span>
                    )
                } <
                ...
                title = "Detener voz" > ⏹️Detener < /button>	 <
                button onClick = {
                    next
                } > Siguiente⟶ < /button> {
                    showCloze && ( <
            ...
            tab === "banco" && ( <
                div className = "bank-section" > {
                    edit ? ( <
                        div className = "edit-form" >
                        <
                        textarea className = "answers-textarea"
                        rows = {
                            6
                        }
                        value = {
